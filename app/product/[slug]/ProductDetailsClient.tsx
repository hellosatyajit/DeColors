...
const ProductDetailsClient = ({ product }: { product: IProduct }) => {
  ...
  const [images, setImages] = useState(
    product?.variants.flatMap((item: any) => item.image).slice(0, 5)
  );
  ...
  const [reviewStatus, setReviewStatus] = useState<
    "Yes" | "No" | "NotBuyed" | null
  >(null);

  useEffect(() => {
    const fetchProducts = async () => {
      if (product?.brand && product?.category) {
        const { data: Suggestedproducts } = await fetchSuggestedProducts(
          product.brand,
          product.category
        );
  ...
  useEffect(() => {
    const checkReviewStatus = async () => {
      if (session?.user?.email && product?._id) {
        const response = await CheckReview(
          session.user.email,
  ...
  const handleVariantSelection = (variant: any) => {
    ...
    if (!variant.attribute.flavor) {
      ...
      let otherImages = product.variants
        .flatMap((v: any) => v.image)
        .filter((url: string) => !selectedVariantImages.includes(url));
  ...
  const handleSelectChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    const selectedVariant = product.variants.find(
      (variant: any) => variant.sku === selected
    );
  ...
  const shareOnFacebook = () => {
    const url = `https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(
      currentUrl
    )}&quote=Check ${productname} from ${currentUrl}`;
            ...
            <div className="mb-10">
              ...
              {showError && (
                <div className="text-red-600 mt-1">
                  {variantType === "color" ? "Color" : "Flavor"} selection is
                  required
                </div>
              )}
            </div>
            ...
            <div>
              ...
              <div className="mb-10 flex space-x-2 text-white">
                <button
                  ...
                  onClick={() =>
                    ShareLinks.shareOnFacebook(currentUrl, product.name)
                  }
                >
                  <svg
                ...
                <button
                  ...
                  onClick={() =>
                    ShareLinks.shareOnWhatsApp(currentUrl, product.name)
                  }
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
              ...
              <>
                ...
                {reviewStatus === "NotBuyed" && (
                  <div className="text-red-600 mb-4">
                    You can only review after the purchase of the product.
                  </div>
                )}
                {reviewStatus === null && (
